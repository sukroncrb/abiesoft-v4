...
let jumlahDataLama = 0; // Untuk menyimpan jumlah data sebelumnya
let loadAwal = true; 
let dataPerangkatLain = [];

function loadDataLog() {
    getData('users/log').then(result => {
        dataPerangkatLain = result.data;
        let jumlahDataBaru = dataPerangkatLain.length;
        if (loadAwal || jumlahDataBaru !== jumlahDataLama) {
            //console.log("Data berubah atau load awal, memperbarui UI...");
            setDataLog();
            jumlahDataLama = jumlahDataBaru;
            loadAwal = false;
        } else {
            // console.log("Tidak ada perubahan data, skip refresh DOM.");
        }
    });
}

function setDataLog() {
    let logPerangkatIni = document.getElementById("logPerangkatIni");
    let logPerangkatLain = document.getElementById("logPerangkatLain");
    let logCleaner = document.getElementById("logCleaner");
    let viewLog = document.getElementById("viewLog");

    if (!logPerangkatIni) return;

    let kontenLogPerangkatIni = "";
    let kontenLogPerangkatLain = "";

    logPerangkatLain.innerHTML = "";
    logCleaner.innerHTML = "";

    const shimmerTemplate = `
        <div class="flex items-start gap-4 p-4 border border-slate-100 rounded-xl mb-4">
            <div class="w-12 h-12 rounded-lg shimmer flex-shrink-0"></div>
            <div class="flex-1">
                <div class="h-4 w-1/3 rounded shimmer"></div>
                <div></div>
                <div class="h-3 w-1/2 rounded shimmer"></div>
                <div></div>
                <div class="h-2 w-1/4 rounded shimmer"></div>
            </div>
        </div>
    `;

    logPerangkatIni.innerHTML = shimmerTemplate;
    logPerangkatLain.innerHTML = shimmerTemplate;

    setTimeout(()=>{
        for (let i = 0; i < dataPerangkatLain.length; i++) {
            if (dataPerangkatLain[i].inisial == viewLog.dataset.inisial) {
                kontenLogPerangkatIni += `
                    <div id class="flex flex-col sm:flex-row sm:items-center justify-between p-4 border border-blue-200 rounded-xl bg-blue-50/30 gap-4 animate-in fade-in duration-500">
                        <div class="flex items-start gap-4">
                            <div class="p-2.5 bg-white rounded-lg shadow-sm border border-slate-200 text-blue-600 flex-shrink-0">
                                <i data-lucide="${dataPerangkatLain[i].tipe}" class="w-6 h-6"></i>
                            </div>
                            <div>
                                <h4 class="font-bold text-slate-800 text-sm flex items-center gap-2 flex-wrap">
                                    ${dataPerangkatLain[i].device}
                                    <span class="bg-green-100 text-green-700 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide">Perangkat Ini</span>
                                </h4>
                                <p class="text-xs text-slate-500 mt-1">${dataPerangkatLain[i].lokasi} • ${dataPerangkatLain[i].ip}</p>
                                <p class="text-[10px] font-medium text-slate-400 mt-1 flex items-center gap-1">
                                    <span class="w-1.5 h-1.5 rounded-full bg-green-500 inline-block"></span> Aktif Sekarang
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                kontenLogPerangkatLain += `
                    <div id="${dataPerangkatLain[i].inisial}" class="mt-4 flex flex-col sm:flex-row sm:items-center justify-between p-4 border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors gap-4 animate-in fade-in duration-500">
                        <div class="flex items-start gap-4">
                            <div class="p-2.5 bg-white rounded-lg shadow-sm border border-slate-200 text-slate-500 flex-shrink-0">
                                <i data-lucide="${dataPerangkatLain[i].tipe}" class="w-6 h-6"></i>
                            </div>
                            <div>
                                <h4 class="font-bold text-slate-800 text-sm">${dataPerangkatLain[i].device}</h4>
                                <p class="text-xs text-slate-500 mt-1">${dataPerangkatLain[i].lokasi} • ${dataPerangkatLain[i].ip}</p>
                                <p class="text-[10px] font-medium text-slate-400 mt-1">Login: ${dataPerangkatLain[i].login}</p>
                            </div>
                        </div>
                        <button onclick="keluarkanPerangkat(this)" data-inisial="${dataPerangkatLain[i].inisial}" data-label="${dataPerangkatLain[i].device}" class="text-sm text-red-600 hover:text-red-700 font-semibold px-4 py-2 bg-red-50 hover:bg-red-100 rounded-lg transition-colors sm:w-auto w-full text-center">
                            Keluarkan
                        </button>
                    </div>
                `;
        ...
        logPerangkatIni.innerHTML = kontenLogPerangkatIni;
        logPerangkatLain.innerHTML = kontenLogPerangkatLain;
        
        if(kontenLogPerangkatLain != ""){
            logCleaner.innerHTML = `
            <div class="pt-4 mt-2 border-t border-slate-100 flex justify-end">
                <button onclick="keluarkanSemuaPerangkat()" class="text-sm text-slate-600 hover:text-slate-800 font-medium transition-colors hover:underline">
